...
local CustomImageManagerAssets = {
    TransparencyTexture = {
        ...
        Path = "Alwayswin/Assets/TransparencyTexture.png",
    ...
    SaturationMap = {
        RobloxId = 4155801252,
        Path = "Alwayswin/Assets/SaturationMap.png",
...
local Library = {
    ...
    Scheme = {
        ...
        MainColor = Color3.fromRGB(189, 172, 255),
...
local MenuScreenSize = Vector2.new()
local MenuScreen = Instance.new("ScreenGui")
MenuScreen.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
MenuScreen.Parent = game:GetService("CoreGui")
MenuScreenSize = MenuScreen.AbsoluteSize
...
Notifications_Frame.Parent = MenuScreen
...
function Library.Notify(Content: string, Delay: number)
    ...
    Text.Position = UDim2.new(0.5, -100, 1, -150 - (NotificationCount * 15))
    ...
    task.spawn(function()
        local Steps = 33
        for i = 1, Steps do
            Text.Position = Text.Position + UDim2.fromOffset(100 / Steps, 0)
            RunService.RenderStepped:Wait()
        end
    end)
    ...
    function Notification:Destroy()
        ...
        for _, v in pairs(Notifications) do
            local self = v.self
            self.Position = self.Position + UDim2.fromOffset(0, 15)
            Index = Index + 1
        end
    end
...
function Library.Indicators(): Indicators
    ...
    do
        ...
        Frame.InputBegan:Connect(function(Input, Process)
            if Process then return end  -- Don't drag if clicking on a text box or other UI element
            
            -- Support both mouse click and touch
            if not Dragging.True and (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
        ...
        UserInput.InputChanged:Connect(function(Input, Process)
            if Dragging.Gui ~= Frame or not Dragging.True then return end
            
            if not (UserInput:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or UserInput.TouchEnabled) then
                if Input.UserInputState == Enum.UserInputState.End then
            ...
            if Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                local ScreenSize = MenuScreenSize
        ...
        Frame.InputEnded:Connect(function(Input, Process)
    ...
    function Indicators.Add(Name: string, Type: string, Value: string, ...): Indicator
        ...
        if Type == "Bar" then
            ...
            ValueLabel.BackgroundColor3 = Library.Scheme.AccentColor
            ValueLabel.Text = ""
    ...
    do
        ...
        if WindowInfo.Icon then
            WindowIcon = New("ImageButton", {
                Image = string.format("rbxassetid://%s", tostring(WindowInfo.Icon)),
